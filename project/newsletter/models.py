...
class Newsletter(models.Model):
    ...
    titre_dossier = models.CharField(
        max_length=255, null=True, blank=True, verbose_name='Titre')
    ...
    lienFace = models.URLField(
        max_length=250, default='http://www.facebook.com/aufinternational')
...
class Fil(models.Model):
    numero = models.IntegerField(max_length=11, default=0)
    bureau = models.ManyToManyField(Region, null=False, blank=False)
    date = models.DateField()
...
class Planete(models.Model):
    ...
    date = models.DateField()
    titre_dossier = models.CharField(
        max_length=255, null=False, blank=False, verbose_name='Titre')
...
class ProjetPlanete(models.Model):
    ...
    photo_projet = models.ImageField(
        upload_to='planete', null=True, blank=True)
...
class MembrePlanete(models.Model):
    ...
    titre_membre = models.CharField(
        max_length=255, null=False, blank=False, verbose_name='Titre')
    texte_membre = models.TextField(null=False, blank=False)
    photo_membre = models.ImageField(
        upload_to='planete', null=True, blank=True)
...
class Breve(models.Model):
    ...
    texte_autre = models.TextField(
        blank=True, verbose_name='Autres informations importantes')
    footer = models.TextField(verbose_name='Texte pied de page',
                              default='Les brèves des services centraux sont une publication réalisée par l\'Agence universitaire de la Francophonie. L\'AUF, opérateur de la Francophonie, est un réseau mondial de 800 établissements d\'enseignement supérieur et de recherche.')
    ...
    def clean(self):
        ...
        if Abonne.objects.filter(adresse=cleaned_data.get("adresse")):
            raise forms.ValidationError(
                "Vous êtes déja inscrit à la lettre d\'information")
...
class DesinscireForm(forms.Form):
    ...
    def clean(self):
        ...
        if not Abonne.objects.filter(adresse=cleaned_data.get("adresse")):
            raise forms.ValidationError(
                "Vous êtes pas inscrit à la lettre d\'information")
...
class VideoPlugin(CMSPlugin):
    ...
    video = models.TextField(
        null=True, blank=True, verbose_name='Code de la vidéo')
    description = models.TextField(
        null=True, blank=True, verbose_name='Description de la vidéo')
