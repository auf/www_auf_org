...
"""
    ...
    def render_tag(self, context, name, extra_bits, nodelist=None):
        content = Placeholder.render_tag(self, context, name, extra_bits, nodelist)
        context.push()
...
"""
	...
	#lang = get_language_from_request(context['request'])
        content =  _show_placeholder_for_page(context, self.name, self.page_lookup.resolve(context), lang=None, site=None, cache_result=False)['content']
        context.push()
        context[self.var] = mark_safe(content)
        output = self.nodelist.render(context)
        context.pop()
...
def do_show_placeholdervar(parser, token):
    ...
    try:
        ...
        if len(bits) == 6:
            if bits[3].lower() != "inherit":
                raise template.TemplateSyntaxError("Parameter 3 must be 'inherit' when using 5 parameters")
            inherit = True
    ...
    #return ShowPlaceholdervarNode(name=bits[1], var=bits[-1], page_lookup=28, nodelist=nodelist, inherit=inherit)
