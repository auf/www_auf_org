...
class MembreFilter(django_filters.FilterSet):
    region = django_filters.ModelChoiceFilter(
        label="Par implantation régionale de l'AUF", empty_label="Sélectionnez une région...", queryset=Region.objects.all())
    pays = django_filters.ModelChoiceFilter(
        label="Par Pays", empty_label="Sélectionnez un pays...", queryset=Pays.objects.all())
    statut = django_filters.ChoiceFilter(label="Par statut de membre", required=False, choices=(
        ('', 'Sélectionnez un statut...'), ('T', 'Membre Titulaires'), ('A', 'Membres associés')))
...
class ImplantationFilter(django_filters.FilterSet):
    ...
    type = django_filters.ChoiceFilter(label="Type", required=False, choices=[(
        '', '-----------')] + [(x, x) for x in list(set(Implantation.ouvertes.values_list('type', flat=True)))])
