...
class ResponsableInline(admin.StackedInline):
    ...
    extra = 0
    fields = ('salutation', 'nom', 'prenom', 'courriel', 'fonction', 'sousfonction', )
    formfield_overrides = {
    ...
    def get_formset(self, request, obj=None, **kwargs):
        formset = super(ResponsableInline, self).get_formset(request, obj, **kwargs)
...
class ResponsablePHAInline(ResponsableInline):
    ...
    def queryset(self, request):
        ...
        queryset = queryset.filter(type=espace_membre.RESPONSABLE_ETABLISSEMENT)
        return queryset
...
class ResponsableComInline(ResponsableInline):
    ...
    def queryset(self, request):
        ...
        queryset = queryset.filter(type=espace_membre.RESPONSABLE_COMMUNICATION)
        return queryset
    ...
    fieldsets = (
            ...
            (u"Validation", {
                'fields': (
                    ...
                    ('validation_etablissement', 'date_validation_etablissement'),
                    ('export_gde_sai', 'export_gde_com'),
                    )
                }
            ),
    )
    readonly_fields = ('date_validation_sai', 'date_validation_etablissement', 'date_validation_com')
